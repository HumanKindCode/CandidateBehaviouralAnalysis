...
def get_llm_response_openness(openness_text):
    ...
    llm = ChatOpenAI(openai_api_key="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")
...
def create_and_run_seq_chain(type, candidate_response, llm, negative_criteria_details, evaluation_prompt_template):
    ...
    for criterion, details in negative_criteria_details.items():
        ...
        time.sleep(1)  # Adjust sleep time based on actual processing time
        if type == "openness":
    ...
    status_message.text("Processing complete.")
        # st.write(chain.prompt)
        # st.write(f"Created chain for {criterion}")
...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["Candidate Answers", "Openness", "Conscientiousness", "Extraversion", "Agreebleness", "Neuroticism","Compare Candidates"])
    ...
    with tab7:
        ...
        # Streamlit UI
        candidate_names = existing_data['CandidateName'].unique()
        selected_candidates = st.multiselect('Select candidates', candidate_names)
